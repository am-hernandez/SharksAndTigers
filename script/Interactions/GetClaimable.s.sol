...
/**
 * @title GetClaimable
 * @notice Read-only script to print a player's claimable reward balance from EscrowManager.
 * @dev Usage: make get-claimable PLAYER=1 (or PLAYER=2).
 */
contract GetClaimable is DeploymentHelper {
    function run(uint8 player) external view {
        ...
        uint256 amount = escrowManager.claimable(playerAddress);

        console.log("Player", uint256(player));
        console.log("Player address:", playerAddress);
        console.log("Claimable (raw):", amount);
        console.log("Claimable (USDC):", amount / 1e6);
    }
}
