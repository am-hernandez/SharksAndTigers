...
contract SharksAndTigersFactory {
    ...
    function createGame(
        ...
        SharksAndTigers game = (new SharksAndTigers){value: msg.value}(
            msg.sender,
            position,
            playerOneMark,
            playClock,
...
contract SharksAndTigers {
    ...
    function joinGame(
        uint position
    ) external payable validatePlayerMove(position) {
    ...
    function makeMove(uint position) public validatePlayerMove(position) {
        ...
        require(
            block.timestamp - lastPlayTime <= playClock,
            "You ran out of time to make a move"
        );
    ...
    function withdrawWager() public {
        require(
            gameState != GameState.Active,
            "Cannot withdraw wager while game is active"
        );
        require(
            winner == address(0),
            "Game is not a draw, winner must call claimReward"
        );
